...
function TypeBadge({ type }: { type: string }) {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${isMonthly
...
function SortIcon({ field, sortBy, dir }: { field: string; sortBy: string; dir: 'asc' | 'desc' }) {
    if (sortBy !== field) return <ChevronsUpDown className="h-3 w-3 text-zinc-300" />;
    return dir === 'asc'
        ...
        <Card className="border border-zinc-100 dark:border-zinc-800 shadow-sm bg-white dark:bg-zinc-900">
            <CardHeader className="px-6 pt-6 pb-4 border-b border-zinc-50 dark:border-zinc-800">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <CardTitle className="text-lg font-black italic tracking-tight">All Donations</CardTitle>
                        <p className="text-xs text-zinc-500 font-medium mt-0.5">{filtered.length} records</p>
                    </div>
                        ...
                        <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl">
                            {(['ALL', 'ONE_TIME', 'MONTHLY'] as const).map((t) => (
                                <button
                                    ...
                                    className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${typeFilter === t
                                        ? 'bg-white dark:bg-zinc-700 text-brand shadow-sm'
                                        : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200'
                                        }`}
                                >
                                    {t === 'ONE_TIME' ? 'One-Time' : t === 'MONTHLY' ? 'Monthly' : 'All'}
                                </button>
                            ))}
                        </div>
            ...
            <div className="overflow-x-auto">
                <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-6 py-3 bg-zinc-50 dark:bg-zinc-800/50">
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Donor</span>
                    <button
                        onClick={() => onSort('date')}
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Type</span>
                    <button
                ...
                <div className="divide-y divide-zinc-50 dark:divide-zinc-800/50">
                    {filtered.length === 0 ? (
                        <div className="py-16 text-center">
                            <p className="font-bold text-zinc-400 text-sm">No donations match your filters.</p>
                        </div>
                    ) : (
                        filtered.map((d) => (
                            <div
                            ...
                            >
                                <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <p className="font-bold text-sm text-zinc-900 dark:text-zinc-100 truncate">{d.name}</p>
                                </div>
                                <p className="text-xs font-medium text-zinc-400 whitespace-nowrap">{formatDate(d.date)}</p>
                                <TypeBadge type={d.type} />
                                <p className="font-black text-sm text-zinc-900 dark:text-zinc-100 tabular-nums text-right whitespace-nowrap">
                                    ${d.amount}{d.type === 'MONTHLY' ? '/mo' : ''}
                                </p>
                            </div>
                        ))
                    )}
