...
export function useManagePrayers() {
    ...
    const prayers: Prayer[] = Array.isArray(data) ? data : (data as any)?.results ?? [];
    ...
    const statusMutation = useMutation({
        ...
        onSuccess: (updated) => {
            notify.success(`Prayer marked as ${STATUS_CONFIG[updated.status].label}.`);
            queryClient.invalidateQueries({ queryKey: ['admin', 'prayers'] });
        },
    ...
    const handleMarkPrayed = useCallback(
        (prayer: Prayer) => statusMutation.mutate({ id: prayer.id, status: 'PRAYED' }),
        [statusMutation]
    );
