...
import { ChevronDown, ChevronRight, Trash2, Loader2, AlertCircle } from 'lucide-react';
...
function PrayerTableRow({
    prayer, isExpanded, isUpdating,
    onToggle, onMarkPrayed, onStatusChange, onSendFollowUp, onDelete,
}: PrayerTableRowProps) {
    return (
        <>
            <TableRow
                onClick={onToggle}
                className={`border-zinc-100 dark:border-zinc-800 h-20 cursor-pointer transition-colors ${isExpanded
            ...
            >
                ...
                <TableCell>
                    <p className={`font-bold ${isExpanded ? 'text-brand' : 'text-zinc-900 dark:text-zinc-100'}`}>
                        {prayer.title}
                    </p>
                    <p className="text-xs text-zinc-400 font-medium mt-0.5">
                        {prayer.isAnonymous ? 'Anonymous' : prayer.email}
                        {prayer.assigned_to_email && (
                            <span className="ml-1 text-blue-500">→ {prayer.assigned_to_email}</span>
                        )}
                    </p>
                </TableCell>
                <TableCell>
...
export function PrayerTable({
    filtered, isLoading, expandedId,
    statusMutationPending, statusMutationVariableId,
    search, statusFilter,
    onToggle, onMarkPrayed, onStatusChange, onSendFollowUp, onDelete,
}: PrayerTableProps) {
    return (
            ...
            <Table>
                <TableHeader className="bg-zinc-50 dark:bg-zinc-800/50">
                    <TableRow className="hover:bg-transparent border-zinc-100 dark:border-zinc-800">
                        ...
                        {['Prayer Request', 'Category', 'Status', 'Date', 'Actions'].map((h, i) => (
                            <TableHead
                                key={h}
                ...
                <TableBody>
                    ...
                    {!isLoading && filtered.map((prayer) => (
                        <PrayerTableRow
                            ...
                            isUpdating={statusMutationPending && statusMutationVariableId === prayer.id}
