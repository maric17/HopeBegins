                    ...
                    <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl w-full sm:w-fit">
                        {(['ALL', 'PENDING', 'ACTIVE'] as const).map((status) => (
                            <button
                                ...
                                className={`flex-1 sm:flex-none px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${filterStatus === status
                    ...
                    {filteredCarriers.length > 0 ? (
                        filteredCarriers.map((carrier) => (
                            <CarrierCard
                                ...
                                isApproving={approveMutation.isPending && approveMutation.variables === carrier.id}
                            />
                        ))
                    ) : (
                        <div className="py-20 text-center space-y-4">
                            ...
                            <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 italic">No carriers found</h3>
